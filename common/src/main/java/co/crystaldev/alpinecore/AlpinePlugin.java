...
public abstract class AlpinePlugin extends JavaPlugin implements Listener {
    ...
    public static @NotNull TaskScheduler scheduler() {
        TaskScheduler local = SCHEDULER;
        if (local == null) {
            synchronized (AlpinePlugin.class) {
                local = SCHEDULER;
                if (local == null) {
                    local = SCHEDULER = Platform.get().createScheduler(Bukkit.getServer());
                }
            }
        }
        return local;
    ...
    private void setupCommandManager() {
        ...
        Platform.get().configureCommands(this, builder, this.miniMessage);
     ...
     *
     * @return whether the server is region-threaded
     * @since 0.5.0
     */
    public static boolean isRegionized() {
        return Platform.get().isRegionized();
    }
